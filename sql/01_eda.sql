...
SELECT DISTINCT(dt.ticket_type_name) AS ticket_name, COUNT(*) AS number_of_visits
FROM fact_visits fv
...
SELECT 
  ...
  AVG(fact_ride_events.satisfaction_rating) AS satisfaction_rating
FROM dim_attraction
...
SELECT DISTINCT(dte.day_name) AS day_of_week, AVG(vist.party_size) AS avg_party_size
